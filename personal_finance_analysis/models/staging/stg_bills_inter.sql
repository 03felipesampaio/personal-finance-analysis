with source as (
    select
        ...
        case
            when
                transaction_description like 'Pagamento On Line'
                then 'Bill payment'
            when
                transaction_description like 'PAGAMENTO ON LINE'
                then 'Bill payment'
            when
                transaction_description like 'PAGTO DEBITO AUTOMATICO'
                then 'Bill payment'
            when
                transaction_description like 'Pagto Debito Automatico'
                then 'Bill payment'
            -- Tem muita informação no Ifood que dá pra extrair
            -- when regexp_contains(transaction_description, r' - \d+/\d+') 
            -- then 'Credit card parcell'
