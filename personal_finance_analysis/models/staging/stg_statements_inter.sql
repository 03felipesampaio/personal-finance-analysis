with source as (
    select
        ...
        case
            ...
            when
                transaction_description like 'Inter Conservador Plus FIRF LP'
                then 'Investment'
            when
                transaction_description like 'LCI LIQ 90 DIAS BANCO INTER SA'
                then 'Investment'
            ...
            when
                transaction_description like '% crédito debitado %'
                then 'Debit card'
            when
                transaction_description like 'Débito Automático Fatura Cartão Inter'
                then 'Bill payment'
            when transaction_description like 'Pagamento' then 'Bill payment'
            when
                transaction_description like 'Pagamento Fatura Cartão Inter'
                then 'Bill payment'
            when
                transaction_description like 'Pagamento efetuado'
                then 'Bill payment'
            when
                transaction_description like 'Pagamento efetuado fatura cartão Inter'
                then 'Bill payment'
            when
                transaction_description like 'Pagamento fatura cartão Inter'
                then 'Bill payment'
            when
                transaction_description like 'Débito automático -%'
            ...
            when
                transaction_description like 'Cp :%' and transaction_value < 0
                then 'Pix sent'
            when
                transaction_description like 'Pix recebido: %'
                then 'Pix received'
            when
                transaction_description like 'Cp :%' and transaction_value >= 0
                then 'Pix received'
